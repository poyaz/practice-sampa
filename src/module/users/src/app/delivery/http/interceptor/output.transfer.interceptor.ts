...
import { HTTP_DATETIME_INFRA_INJECT_TOKEN, HTTP_LOGGER_INJECT_TOKEN } from '@users-delivery/http/users.constant';
import { ExceptionEnum } from '@users-domain/errors/exception.enum';
import { IDateTimeAdapter } from '@adapter/date-time.adapter';

@Injectable()
export class OutputTransferInterceptor implements NestInterceptor {
  constructor(
    @Inject(HTTP_DATETIME_INFRA_INJECT_TOKEN)
    private readonly _dateTime: IDateTimeAdapter,
    @Inject(HTTP_LOGGER_INJECT_TOKEN)
